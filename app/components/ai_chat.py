...
def render_ai_chat_page() -> None:
    ...
    valid_use_cases = {"market_signals", "entry_strategy", "avoid_warnings", "idea_validator"}
    ...
    if "validator_inputs" not in st.session_state:
        st.session_state.validator_inputs = {}

    st.markdown("## AI Market Advisor")
    st.write(
        "Use Bangkok Airbnb market data to discover market signals, entry strategies, risks, "
        "and validate your property idea."
    )
    st.divider()
    ...
    st.markdown("### Validate Your Airbnb Idea")

    hoods = _get_neighbourhoods()
    
    val_col1, val_col2, val_col3 = st.columns(3)
    with val_col1:
        selected_hood = st.selectbox("Neighbourhood", options=hoods, index=0)
    with val_col2:
    ...
    with val_col3:
        target_price = st.number_input(
            "Planned Nightly Price",
            min_value=100,
            max_value=50000,
            value=1500,
            step=100,
            help="Nightly price in THB.",
        )

    if st.button("Verify Property Idea", use_container_width=True):
        st.session_state.active_use_case = "idea_validator"
        st.session_state.validator_inputs = {
            "neighbourhood": selected_hood,
            "room_type": selected_rt,
    ...
    if st.session_state.active_use_case:
        ...
        kwargs = {}
        if uc == "idea_validator":
            kwargs = st.session_state.validator_inputs
        ...
        elif uc == "idea_validator":
            _render_idea_validator(result)
