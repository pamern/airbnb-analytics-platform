...
with latest_price_predictions as (
    select
        listing_id,
        predicted_price
    from gold.gold_listing_price_predictions
    qualify row_number() over (
        partition by listing_id
        order by predicted_at desc nulls last, run_id desc nulls last
    ) = 1
),

latest_cluster_assignments as (
    select
        listing_id,
        cluster_name
    from gold.gold_listing_cluster_assignments
    qualify row_number() over (
        partition by listing_id
        order by assigned_at desc nulls last, run_id desc nulls last
    ) = 1
),

listing_market_ml as (
    select
        ...
        case
            when fact.estimated_occupancy_l365d between 0 and 365
                then fact.estimated_occupancy_l365d / 365.0
    ...
    left join latest_price_predictions as price_predictions
        on fact.listing_id = price_predictions.listing_id
    left join latest_cluster_assignments as cluster_assignments
        on fact.listing_id = cluster_assignments.listing_id
