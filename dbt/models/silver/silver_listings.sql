...
renamed as (
    select
        ...
        try_cast({{ source_column(listings_source, 'estimated_occupancy_l365d', default='NULL') }} as integer) as estimated_occupancy_l365d,
        {{ clean_money(source_column(listings_source, 'estimated_revenue_l365d', default='NULL')) }} as estimated_revenue_l365d,
        ...
        try_cast({{ source_column(listings_source, 'number_of_reviews_l30d', default='NULL') }} as integer) as number_of_reviews_l30d,
        try_cast({{ source_column(listings_source, 'number_of_reviews_ltm') }} as integer) as number_of_reviews_ltm,
        try_cast({{ source_column(listings_source, 'number_of_reviews_ly', default='NULL') }} as integer) as number_of_reviews_ly,
